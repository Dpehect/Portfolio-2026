...
const setup = ({
  ...
  tlLanguages: gsap.core.Timeline;
  contentLanguages: HTMLDivElement;
...
const setupSectionsAnimation = ({
  ...
  sectionsMm = createMatchMedia((_context, { isLandscape }) => {
    ...
    if (isLandscape) {
      ...
      // Languages animation
      tl.fromTo(contentLanguages, { opacity: 0 }, { opacity: 1, duration: 0.15, ease: "power1.out" }, LANGUAGES_DELAY);
      tl.add(() => {
        tlLanguages?.play();
      }, LANGUAGES_DELAY);
    } else {
      ...
      tl.to(contentServices, { opacity: 0, y: "-10vh", duration: 0.15, ease: "power1.out" }, LANGUAGES_DELAY - 0.075);
      tl.add(() => {
        tlServices?.play();
      }, SERVICES_DELAY);
